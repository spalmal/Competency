...
<div id="col1">
  ...
  <form  onsubmit="this.action = '/mvcchart/getAssociates/' + this.name.value"   method="get">
  <span> Select Mentor Name : </span>
 <select id="name">
 <option value="select">Select Mentor Name</option>
 <c:forEach var="listValue" items="${mentors}">
			<h4><option value='${listValue.employeeId}'>${listValue.employeeFirstName}</option> </h4>
			</c:forEach>
 </select>

 <input type="submit" value="Get Details"></input>
 </form>
<c:if test="${not empty lineChart}">
		 <img alt="Google Pie Chart" src=${lineChart} />
	</c:if>
  <div class='box'>
    <div class='boxtop'>
      <div></div>
